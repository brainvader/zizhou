...
- すべてのコード変更は `git apply --ignore-whitespace` で適用可能な unified diff 形式（patchファイル）で提供する
- patchファイルは必ずローカル環境で `git apply --check` による検証を行ってから提示する
- 新規ファイルの場合のみ完全ファイルで提供する（patchが存在しないため）
- patchファイル名は `ctx{N}-{対象ファイル名}.patch` の形式とする
