...
```
ContextMap.html          何を作るか・画面構成・コンテキストの分解（LLMとの対話で育てる）
      ↓
Vitest spec              コンテキストの具体化（import = スコープ、describe = 境界）
      ↓                  LLM と対話しながらロジック・型・インターフェースを固める
実装                     spec の import が実装すべきものをすべて示している
      ↓                  人間が vitest run で確認し、失敗は LLM と対話して修正
Storybook stories        実際に動くか視覚確認。play 関数でインタラクション検証
      ↓
